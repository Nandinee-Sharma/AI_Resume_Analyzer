...
def extract_keywords(text, top_n=10):
    ...
    stopwords = set(['with', 'from', 'that', 'have', 'this', 'will', 'your', 'about', 'should', 'skills'])
...
if st.session_state.form_submitted:
    ...
    st.markdown(f"""
        <div style='text-align: left; background-color: #000000; color:white; padding: 10px; border-radius: 10px; margin: 5px 0;'>
            {report}
        </div>
    """, unsafe_allow_html=True)

    # 🔍 Keyword Highlighting
    st.subheader("🔍 Keyword Match with Job Description")
    keywords = extract_keywords(st.session_state.job_desc, top_n=10)
    resume_text = st.session_state.resume.lower()

    matched = [kw for kw in keywords if kw in resume_text]
    missing = [kw for kw in keywords if kw not in resume_text]

    st.markdown("**✅ Matched Keywords:** " + ", ".join(
        [f"<span style='color:green'>{kw}</span>" for kw in matched]), unsafe_allow_html=True)
    st.markdown("**❌ Missing Keywords:** " + ", ".join(
        [f"<span style='color:red'>{kw}</span>" for kw in missing]), unsafe_allow_html=True)
